...
export class TextRouter {
  ...
  */
  ...
  public run(context: TextRouterContext): boolean {
    ...
    const isSubMatched = this.subRouters.reduce((isMatched, { condition, subRouter }) => {
      if (isMatched) return true
      const [, matchedText, remain = ''] = TextRouter.match(remainText, condition) ?? []
  ...
  protected static match(
    ...
    return (text.match(regexp)?.slice(0, 3) ?? null) as [string, string, string] | null
  }
